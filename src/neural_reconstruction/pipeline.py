...
from .common.data_types import SeedPoint, ComponentAnalysisResult
...
class NeuralReconstructionPipeline:
    ...
    def _to_global_coords(
        self, result: ComponentAnalysisResult
        ...
        topology_dict = {"nodes": nodes_global, "edges": edges_global}

        return seeds_global, topology_dict

    def _build_mst_with_paths(
        self, mst_forest: nx.Graph, all_connections: List[Dict]
    ) -> Dict:
        """
        ...
        return {"edges": mst_edges_with_paths, "num_edges": len(mst_edges_with_paths)}
    ...
    def run(
        ...
        results = {"output_dir": output_dir, "stages": {}}
        ...
        for result in component_results:
            ...
            all_topologies.append(
                {"component_id": result.component_id, "topology": topology_dict}
            )
        ...
        for topo_data in all_topologies:
            ...
            # 找出屬於這個元件的所有種子
            component_seeds = [
                s for s in all_seeds if s["component_id"] == component_id
            ]

            components_data.append(
                {"component_id": component_id, "seeds": component_seeds}
            )
        ...
        logger.info(
            f"  ✓ 元件圖: {component_graph.number_of_nodes()} 節點, "
        ...
        logger.info(
            f"  ✓ MST 森林: {mst_forest.number_of_nodes()} 節點, "
        ...
        mst_with_paths = self._build_mst_with_paths(
            mst_forest, pairing_results["connections"]
        )
