...
class XgbMstLinker:
    ...
    def __init__(
        ...
        self.xgb_proba_threshold = xgb_proba_threshold

        model = XGBClassifier()
        model.load_model(Path(Path(__file__).parent) / "xgb_connection_filter.json")
        self.model = model

        loaded_scaler = joblib.load(Path(Path(__file__).parent) / "scaler.joblib")
    ...
    def _run_reconstruction(
        ...
        # 3. 元件間路徑查找
        cost_map = (
            (255 - image.astype(np.float64)) / 255.0
        ) ** 8  # 成本地圖，越亮成本越低

        self._add_inter_component_edges(global_graph, cost_map, image)

        # 4. MST
        return nx.minimum_spanning_tree(global_graph, weight="weight")

    def _add_inter_component_edges(
        self, global_graph: nx.MultiGraph, cost_map: np.ndarray, img: np.ndarray
    ) -> None:
        ...
        for i, source in enumerate(topology_points):
            ...
            for target_pos, result in paths.items():
                ...
                distance = float(
                    np.linalg.norm(np.array(source) - np.array(target_pos))
                )
                # calculate path length as sum of Euclidean distances between consecutive points
                path_length = float(
                    np.sum(np.linalg.norm(np.diff(pth_arr, axis=0), axis=1))
                )
                normalized_cost = float(cost / max(path_length, 1e-8))
                intensities = img[pth_arr[:, 0], pth_arr[:, 1]].astype(np.float32)

                # Max perpendicular deviation from the straight line connecting endpoints
                start, end = pth_arr[0].astype(float), pth_arr[-1].astype(float)
                line_vec = end - start
                line_len = np.linalg.norm(line_vec)
                if line_len > 0:
                    max_deviation = float(
                        np.max(np.abs(np.cross(line_vec / line_len, pth_arr - start)))
                    )
                else:
                    max_deviation = 0.0
                ...
                if prob < self.xgb_proba_threshold:  # threshold can be tuned
                    continue

                enhance_weight = (
                    1 - prob
                ) * normalized_cost  # 以模型預測的概率調整成本
                global_graph.add_edge(
                    tuple(source.astype(int)),
                    target_pos,
