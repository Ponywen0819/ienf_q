...
def compute_dgm_force(
    lh_dgm, gt_dgm, pers_thresh=0.03, pers_thresh_perfect=0.99, do_return_perfect=False
):
    """
...
def getCriticalPoints(likelihood):
    ...
    lh_cubic = gd.CubicalComplex(
        dimensions=[lh.shape[0], lh.shape[1]], top_dimensional_cells=lh_vector
    )
...
def getTopoLoss(likelihood_tensor, gt_tensor, topo_size=100):
    ...
    likelihood = torch.squeeze(likelihood_tensor).cpu().detach().numpy()
    gt = torch.squeeze(gt_tensor).cpu().detach().numpy()
    ...
    for y in range(0, likelihood.shape[0], topo_size):
        for x in range(0, likelihood.shape[1], topo_size):
            ...
            gt_patch = gt[
                y : min(y + topo_size, gt.shape[0]), x : min(x + topo_size, gt.shape[1])
            ]
            ...
            force_list, idx_holes_to_fix, idx_holes_to_remove, idx_holes_perfect = (
                compute_dgm_force(pd_lh, pd_gt, pers_thresh=0.03)
            )
            ...
            if len(idx_holes_to_fix) > 0 or len(idx_holes_to_remove) > 0:
                for hole_indx in idx_holes_to_fix:
                    ...
                    if (
                        ...
                        topo_cp_weight_map[
                            y + int(dcp_lh[hole_indx][0]), x + int(dcp_lh[hole_indx][1])
    ...
    topo_cp_weight_map = torch.tensor(topo_cp_weight_map, dtype=torch.float).to(
        likelihood_tensor.device
    )
    topo_cp_ref_map = torch.tensor(topo_cp_ref_map, dtype=torch.float).to(
        likelihood_tensor.device
    )

    # Measuring the MSE loss between predicted critical points and reference critical points.
    # Normalise by the number of active critical points so the loss stays in [0, 1]
    # and is comparable in scale with BCE / Dice.
    n_critical = topo_cp_weight_map.sum()
    if n_critical == 0:
        return torch.tensor(0.0, device=likelihood_tensor.device, requires_grad=True)

    loss_topo = (
        ((likelihood_tensor * topo_cp_weight_map) - topo_cp_ref_map) ** 2
    ).sum() / n_critical
    return loss_topo
