...
對輸入影像進行閉操作（Closing = 膨脹 + 腐蝕），並輸出結果和 Viridis 視覺化
...
使用方式:
python morphology_closing.py --input image.tif --output output.tif --kernel-size 5
python morphology_closing.py --input image.tif --output-dir output/ --visualize
"""
...
def visualize_comparison(
    ...
    axes[1].set_title('After Closing', fontsize=12, weight='bold')
    axes[1].axis('off')

    # 差異圖
    diff = cv2.absdiff(closed, original)
    ...
    if title:
        ...
        fig.suptitle('Morphological Closing Comparison', fontsize=14, weight='bold')
...
def visualize_side_by_side_viridis(
    ...
    # 閉操作結果
    im2 = axes[1].imshow(closed, cmap='viridis')
    axes[1].set_title('After Closing', fontsize=12, weight='bold')
    ...
    if title:
        ...
        fig.suptitle('Morphological Closing - Viridis Visualization', fontsize=14, weight='bold')
...
def main():
    parser = argparse.ArgumentParser(
        description='對影像進行形態學閉操作並視覺化',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
範例:
  # 基本使用 - 輸出閉操作結果
  python morphology_closing.py \\
      --input image.tif \\
      --output output.tif \\
      --kernel-size 5
  ...
  python morphology_closing.py \\
    ...
    try:
        ...
        # 步驟 2: 執行閉操作
        print(f"\n[2/3] 執行形態學閉操作...")
        closed, kernel = morphology_closing(
            image=image,
        ...
        if args.output_dir:
            ...
            # 保存閉操作結果
            closed_path = output_dir / f"{base_name}_closed.tif"
            cv2.imwrite(str(closed_path), closed)
            print(f"  ✓ 已保存閉操作結果: {closed_path}")
            ...
            if args.visualize:
                ...
                # Viridis 閉操作結果
                viridis_closed_path = output_dir / f"{base_name}_closed_viridis.png"
                visualize_viridis(
                    image=closed,
                    output_path=str(viridis_closed_path),
                    title="After Closing (Viridis)",
                    show_colorbar=args.colorbar,
                    dpi=args.dpi
                )
                ...
                viridis_direct_closed = output_dir / f"{base_name}_closed_viridis_direct.png"
                visualize_viridis_direct(closed, str(viridis_direct_closed))
            ...
            if args.viridis_only:
                # 只輸出 Viridis 視覺化（無邊框）
                visualize_viridis_direct(closed, args.output)
            else:
                # 輸出閉操作結果
                cv2.imwrite(args.output, closed)
                print(f"  ✓ 已保存閉操作結果: {args.output}")
